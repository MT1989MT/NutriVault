    ...
    <div className="fixed bottom-0 left-0 right-0 glass-nav border-t border-gray-100/60 z-50">
      <div className="max-w-full mx-auto flex items-center justify-around px-3 h-[68px]">
        {items.map(({ view, icon: Icon, labelKey }) => (
          <button key={view} onClick={() => handleClick(view)} className="flex flex-col items-center justify-center min-w-[64px] min-h-[52px] py-1 px-2 transition-smooth active:scale-90">
            ...
            <span className={`text-[10px] font-semibold mt-1 tracking-wide ${currentView === view ? 'text-[#E07A5F]' : 'text-gray-400'}`}>{t(labelKey)}</span>
          </button>
        ))}
      </div>
